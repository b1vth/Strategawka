...
public class TreechopperEnchantment extends Enchantment implements Listener {

    public TreechopperEnchantment() {
        super(new NamespacedKey(Main.getInst(), "treechopper"));
    }
    ...
    public void onChop(BlockBreakEvent e) {
        ...
        if (e.getBlock().getType().toString().endsWith("_LOG")) {
            if (im.getLore().contains(ChatUtil.chat("&7TreeChopper I"))) {
                Set<Block> blocks = VeinMinerUtil.getConnectedblocksWOOD(e.getBlock());
                for (Block b : blocks) b.breakNaturally();
            }
        }
    }
